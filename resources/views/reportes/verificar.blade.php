...
@section('content')
                                                                ...
                                                                @if ($reporte->rondas->where('numero_ronda_id',$numeroRonda->id)->first()->respuestas->where('pregunta_id',$pregunta->id)->first() != null)
                                                                    @if ($pregunta->tipoPregunta->id == 1)
                                                                        <td class="{{$reporte->rondas->where('numero_ronda_id',$numeroRonda->id)->first()->respuestas->where('pregunta_id',$pregunta->id)->first()->alarma ? 'as-alarma-bg' : ''}}" >
                                                                            {{ $reporte->rondas->where('numero_ronda_id',$numeroRonda->id)->first()->respuestas->where('pregunta_id',$pregunta->id)->first()->opcion->nombre ?? 'sin opcion' }}
                                                                        </td>
                                                                    @elseif ($pregunta->tipoPregunta->id == 3)
                                                                        <td class="{{$reporte->rondas->where('numero_ronda_id',$numeroRonda->id)->first()->respuestas->where('pregunta_id',$pregunta->id)->first()->alarma ? 'as-alarma-bg' : ''}}" >
                                                                            {{ $reporte->rondas->where('numero_ronda_id',$numeroRonda->id)->first()->respuestas->where('pregunta_id',$pregunta->id)->first()->extra ?: 'Sin dato extra' }}
                                                                        </td>
                                                                    @endif
                                                                @else
                                                                    <td>Sin dato</td>
    ...
    <form id="registro" class="kt-form kt-form--label-right" method="POST" action="{{ route('reportes.verificacion', $reporte->id) }}" role="form" enctype='multipart/form-data'>
