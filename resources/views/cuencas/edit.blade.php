    ...
    <div class="row">
        <div class="col-xl-12">
            ...
            <div class="kt-portlet">
                ...
                <!--begin::Form-->
                <form id="registro" class="kt-form kt-form--label-right" method="POST" action="{{ route('cuencas.update',$cuenca->id) }}" role="form" enctype='multipart/form-data'>
                    @csrf
                    {{ method_field('PATCH') }}
                    <div class="kt-portlet__body" style="flex-direction: inherit;">
                        <div class="col-md-6">
                            <h4>Datos de la Cuenca</h4>
                        </div>
                    </div>
                    <div class="kt-portlet__body" style="flex-direction: inherit;">
                        <div class="col-md-5">
                                ...
                                <div class="kt-input-icon kt-input-icon--right">
                                    <select class="form-control" name="cuenca_id" required>
                                        @foreach ($unidades as $unidad)
                                            <option {{$cuenca->unidad_gestion_cuenca_id == $unidad->id ? 'selected' : ''}} value="{{ $unidad->id }}">{{ $unidad->codigo }} - {{ $unidad->nombre }}</option>
                                        @endforeach
                                    </select>
