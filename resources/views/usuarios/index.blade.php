                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-xl-4 order-1 order-xl-2 kt-align-right">
                            @can('create', App\User::class)
                                <a style=" background-color: #ef9f0c; border-color: #ef9f0c; " href="{{ route('usuarios.create')}}" type="button" class="btn btn-success btn-elevate btn-pill">
                                        Nuevo usuario
                                    </a>
                            @endcan
                            ...
                            @foreach ($usuarios as $usuario)
                                <tr data-row="0" class="kt-datatable__row" style="left: 0px;">
                                    <td data-field="OrderID" class="kt-datatable__cell">
                                        <span style="width: 117px;">{{ $usuario->identificacion}}</span>
                                    </td>
                                    <td data-field="OrderID" class="kt-datatable__cell">
                                        <span style="width: 117px;">{{ $usuario->name}}</span>
                                    </td>
                                    <td data-field="OrderID" class="kt-datatable__cell">
                                        <span style="width: 117px; overflow: visible;">{{ $usuario->email}}</span>
                                    </td>
                                    <td data-field="ShipDate" class="kt-datatable__cell">
                                        <span style="width: 117px;">{{ $usuario->rol->nombre}}</span>
                                    </td>
                                    <td data-field="Actions" data-autohide-disabled="false" class="kt-datatable__cell">
                                        <span style="overflow: visible; position: relative; width: 110px;">
                                            <a href="{{ route('usuarios.show', $usuario->id)}}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Consultar"><i class="la la-file"></i></a>
                                            @can('update', $usuario)
                                                <a href="{{ route('usuarios.edit', $usuario->id)}}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Modificar"><i class="la la-edit"></i></a>
                                            @endcan
                                        </span>
                                    </td>
                                </tr>
